...
func TestAssertions_Inequality(t *testing.T) {
	Tests(t).Scenarios().
		...
		Test(func(sut Assertions, scenario []any) {
			mockT, testName := setUpSystemUnderTest(sut)
			lhs := scenario[0]
			rhs := scenario[1]
			...
			sut.Equal(lhs, rhs)
			mockT.shouldFailTest(testName, "Expected", lhs, "to be equal to", rhs)
		})
}
...
func TestAssertions_Equality(t *testing.T) {
	Tests(t).Scenarios().
		...
		Test(func(sut Assertions, scenario []any) {
			...
			sut.NotEqual(lhs, rhs)
			mockT.shouldFailTest(testName, "Expected", lhs, "not to be equal to", rhs)
		})
}
...
func setUpSystemUnderTest(asserts Assertions) (*tMock, string) {
	sut := asserts.(*assertions)
	mockT := mockTesting(sut.T)
	sut.T = mockT
	return mockT, sut.testName
}
